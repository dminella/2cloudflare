                    ...
                    dnsRecords.forEach(dnsRecord => {
                        if (dnsRecord.ResourceRecords) {
                            dnsRecord.ResourceRecords.forEach(value => {
                                const convertedRecord = `${dnsRecord.Name} IN ${dnsRecord.Type} ${value.Value}\n`;
                                appendInConvertedJsonFile(convertedRecord);
                            });
                        } else {
                            const convertedRecord = `${dnsRecord.Name} IN ${dnsRecord.Type} ${dnsRecord.AliasTarget.DNSName}`;
                            appendInConvertedJsonFile(convertedRecord);
                        }
                    });
                });
